package de.derfrzocker.ore.control.impl.v26_1_base.feature.generator;
...
public class OreConfigurationGeneratorHook
        extends MinecraftFeatureGeneratorHook<OreConfiguration, OreFeatureConfiguration> {

    private OreConfigurationGeneratorHook(
    ...
    public static OreConfigurationGeneratorHook createOreHook(
            @NotNull OreControlManager oreControlManager,
    ...
    public static OreConfigurationGeneratorHook createScatteredOreHook(
            @NotNull OreControlManager oreControlManager,
            @NotNull NamespacedKey namespacedKey,
            @NotNull Biome biome) {
        return new OreConfigurationGeneratorHook(
                SCATTERED_ORE,
                oreControlManager,
                "scattered_ore",
                biome,
    ...
    public static OreFeatureConfiguration createDefaultConfiguration(
            @NotNull OreConfiguration configuration,
            @NotNull FeatureGenerator<OreFeatureConfiguration> feature) {
        ...
        for (OreConfiguration.TargetBlockState targetBlockState : configuration.targetStates) {
            ...
            if (targetBlockState.target instanceof AlwaysTrueTest) {
                ...
                ruleTest = new BlockMatchRuleTest(CraftMagicNumbers.getMaterial(getFieldValueFromType(
                        rule,
                        Block.class)));
            } else if (targetBlockState.target instanceof BlockStateMatchTest rule) {
                ruleTest = new BlockStateMatchRuleTest(CraftBlockData.fromData(getFieldValueFromType(
                        rule,
                        BlockState.class)));
            } else if (targetBlockState.target instanceof RandomBlockMatchTest rule) {
                ruleTest = new RandomBlockMatchRuleTest(
                        CraftMagicNumbers.getMaterial(getFieldValueFromType(
                                rule,
                                Block.class)),
                        getFieldValueFromType(rule, Float.TYPE));
            } else if (targetBlockState.target instanceof RandomBlockStateMatchTest rule) {
                ruleTest = new RandomBlockStateMatchRuleTest(
                        CraftBlockData.fromData(getFieldValueFromType(
                                rule,
                                BlockState.class)),
                        getFieldValueFromType(rule, Float.TYPE));
            } else if (targetBlockState.target instanceof TagMatchTest rule) {
                ruleTest = new TagMatchRuleTest(CraftNamespacedKey.fromMinecraft(getFieldValueFromType(
                        rule,
                        TagKey.class).location()));
            } else {
                throw new IllegalArgumentException("Got unexpected rule test from class " +
                                                   targetBlockState.target.getClass());
            }
        ...
        return new OreFeatureConfiguration(
                feature,
                targetListValue,
                new FixedDoubleToIntegerValue(configuration.size),
                new FixedFloatValue(configuration.discardChanceOnAirExposure));
    }
    ...
    private static <V> V getFieldValueFromType(Object from, Class<V> type) {
        ...
        throw new IllegalStateException("Didn't found field with type " +
                                        type +
                                        " from class " +
                                        from.getClass()
                                        +
                                        ", following fields are present " +
                                        Arrays.toString(from.getClass().getDeclaredFields()));
    }

    @Override
    public OreConfiguration createConfig(
            @NotNull OreConfiguration defaultConfiguration,
            @NotNull WorldInfo worldInfo,
            @NotNull Random random,
            @NotNull BlockVector position,
            @NotNull LimitedRegion limitedRegion,
            @NotNull OreFeatureConfiguration configuration) {
        ...
        if (configuration.getTargets() == null) {
            ...
            List<TargetBlockState> targetBlockStates = configuration
                    .getTargets()
            ...
            if (targetBlockStates != null) {
                for (TargetBlockState targetValue : targetBlockStates) {
                    ...
                    if (targetValue.getTarget() instanceof AlwaysTrueRuleTest) {
                        ...
                        ruleTest = new RandomBlockMatchTest(
                                CraftMagicNumbers.getBlock(rule.getMaterial()),
                                rule.getProbability());
                    } else if (targetValue.getTarget() instanceof RandomBlockStateMatchRuleTest rule) {
                        ruleTest = new RandomBlockStateMatchTest(
                                ((CraftBlockData) rule.getBlockData()).getState(),
                                rule.getProbability());
                    } else if (targetValue.getTarget() instanceof TagMatchRuleTest rule) {
                        ruleTest = new TagMatchTest(TagKey.create(
                                net.minecraft.core.registries.Registries.BLOCK,
                                CraftNamespacedKey.toMinecraft(rule.getTag())));
                    } else {
                        throw new IllegalArgumentException("Got unexpected rule test from class " +
                                                           targetValue.getTarget().getClass());
                    }

                    blockStates.add(OreConfiguration.target(
                            ruleTest,
                            ((CraftBlockData) targetValue.getState()).getState()));
                }
            }
        }
        ...
        if (configuration.getDiscardChanceOnAirExposure() == null) {
            ...
            discardChanceOnAirExposure = configuration
