...
public abstract class MinecraftFeatureGeneratorHook<M extends FeatureConfiguration,
        C extends FeatureGeneratorConfiguration>
        extends Feature<M> implements FeatureGeneratorHook<C> {
    ...
    public MinecraftFeatureGeneratorHook(
            Codec<M> codec,
            Feature<M> feature,
        ...
        this.featureGenerator = (FeatureGenerator<C>) oreControlManager
                .getRegistries()
    ...
    public abstract M createConfig(
            @NotNull M defaultConfiguration,
    ...
    public boolean place(FeaturePlaceContext<M> context) {
        ...
        FeatureGeneratorConfiguration configuration = cache.computeIfAbsent(
                context
                        .level()
                        .getMinecraftWorld()
                        .getWorld()
                        .getName(), this::loadConfig);
        if (configuration == null) {
            return feature.place(context);
        }

        CraftLimitedRegion limitedRegion = new CraftLimitedRegion(context.level(), ChunkPos.containing(origin));
        M config = createConfig(
                context.config(),
                context.level().getMinecraftWorld().getWorld(),
                new RandomSourceWrapper.RandomWrapper(context.random()),
                new BlockVector(origin.getX(), origin.getY(), origin.getZ()),
                limitedRegion,
                (C) configuration);
    ...
    private FeatureGeneratorConfiguration loadConfig(String name) {
        Config config = configManager
                .getGenerationConfig(configManager.getOrCreateConfigInfo(name), biome, namespacedKey)
                .orElse(null);
    ...
    private boolean place(
            Optional<ConfiguredFeature<?, ?>> top,
            WorldGenLevel world,
