...
import de.derfrzocker.feature.impl.v26_1_base.extra.OreVeinHandler;
import de.derfrzocker.feature.impl.v26_1_base.feature.generator.EmptyConfigurationGenerator;
import de.derfrzocker.feature.impl.v26_1_base.feature.generator.OreConfigurationGenerator;
import de.derfrzocker.feature.impl.v26_1_base.placement.CountModifier;
import de.derfrzocker.feature.impl.v26_1_base.placement.HeightRangeModifier;
import de.derfrzocker.feature.impl.v26_1_base.placement.RarityModifier;
import de.derfrzocker.feature.impl.v26_1_base.placement.SurfaceRelativeThresholdModifier;
import de.derfrzocker.feature.impl.v26_1_base.placement.SurfaceWaterDepthModifier;
import de.derfrzocker.feature.impl.v26_1_base.value.offset.NMSAboveBottomOffsetIntegerValue;
import de.derfrzocker.feature.impl.v26_1_base.value.offset.NMSBelowTopOffsetIntegerValue;
...
import de.derfrzocker.ore.control.impl.v26_1_base.feature.generator.EmptyConfigurationGeneratorHook;
import de.derfrzocker.ore.control.impl.v26_1_base.feature.generator.OreConfigurationGeneratorHook;
import de.derfrzocker.ore.control.impl.v26_1_base.placement.ActivationModifierHook;
import de.derfrzocker.ore.control.impl.v26_1_base.placement.CountModifierHook;
import de.derfrzocker.ore.control.impl.v26_1_base.placement.HeightRangeModifierHook;
import de.derfrzocker.ore.control.impl.v26_1_base.placement.RarityModifierHook;
import de.derfrzocker.ore.control.impl.v26_1_base.placement.SurfaceRelativeThresholdModifierHook;
import de.derfrzocker.ore.control.impl.v26_1_base.placement.SurfaceWaterDepthModifierHook;
import java.io.File;
...
public class NMSReplacer_v26_1_base implements NMSReplacer {
    ...
    public NMSReplacer_v26_1_base(
            @NotNull Plugin plugin,
            @NotNull OreControlManager oreControlManager,
    ...
    private void registerValueTypes() {
        registries
                .getValueTypeRegistry(IntegerType.class)
                .register(new AboveBottomOffsetIntegerType(registries, NMSAboveBottomOffsetIntegerValue::new));
        registries
                .getValueTypeRegistry(IntegerType.class)
                .register(new BelowTopOffsetIntegerType(registries, NMSBelowTopOffsetIntegerValue::new));
    }
    ...
    private void registerFeatures() {
        Registry<PlacedFeature> placedFeatureRegistry = getRegistry().lookupOrThrow(Registries.PLACED_FEATURE);
        Registry<Feature<?>> featureRegistry = getRegistry().lookupOrThrow(Registries.FEATURE);
        Registry<PlacementModifierType<?>>
                placementModifierTypes
                = getRegistry().lookupOrThrow(Registries.PLACEMENT_MODIFIER_TYPE);

        for (PlacedFeature placedFeature : placedFeatureRegistry) {
            Feature<?> feature = placedFeature.getFeatures().findFirst().get().value().feature();
            Identifier resourceLocation = featureRegistry.getKey(feature);
            Optional<FeatureGenerator<?>> featureGenerator = registries
                    .getFeatureGeneratorRegistry()
                    .get(NamespacedKey.fromString(resourceLocation.toString()));
            ...
            for (PlacementModifier modifier : placedFeature.placement()) {
                ...
                Identifier placementModifierType = placementModifierTypes.getKey(modifier.type());
                Optional<FeaturePlacementModifier<?>> modifierOptional = registries
                        .getPlacementModifierRegistry()
                        .get(NamespacedKey.fromString(placementModifierType.toString()));
            ...
            if (!hasCount) {
                Optional<FeaturePlacementModifier<?>> modifierOptional = registries
                        .getPlacementModifierRegistry()
                        .get(NamespacedKey.fromString("minecraft:count"));
                modifierOptional.ifPresent(modifiers::add);
            }
            ...
            Optional<FeaturePlacementModifier<?>> modifierOptional = registries
                    .getPlacementModifierRegistry()
                    .get(ActivationModifier.KEY);
            modifierOptional.ifPresent(modifiers::add);

            registries
                    .getFeatureRegistry()
                    .register(new de.derfrzocker.feature.api.Feature(
                            NamespacedKey.fromString(placedFeatureRegistry
                                                             .getKey(placedFeature)
                                                             .toString()), featureGenerator.get(), modifiers));
        }
    }
    ...
    private void registerBiomes() {
        ...
        forEachBiome((biome, resourceLocation) -> {
            de.derfrzocker.ore.control.api.Biome
                    bio
                    = new de.derfrzocker.ore.control.api.Biome(CraftNamespacedKey.fromMinecraft(resourceLocation));

            biome.getGenerationSettings().features().forEach(featureSet -> featureSet.forEach(holder -> {
                PlacedFeature feature = holder.value();
                Identifier featureKey = placedFeatureRegistry.getKey(feature);

                if (featureKey == null) {
                    // Skip if the feature is not registered, this can happen if another plugin adds a feature
                ...
                registries
                        .getFeatureRegistry()
                        .get(CraftNamespacedKey.fromMinecraft(featureKey))
                        .ifPresent(value -> bio.getFeatures().add(value));
            }));
    ...
    public Set<de.derfrzocker.ore.control.api.Biome> getBiomes(World world) {
        ...
        Registry<Biome> registry = getRegistry().lookupOrThrow(Registries.BIOME);

        for (Holder<Biome> biomeHolder : level.getChunkSource().getGenerator().getBiomeSource().possibleBiomes()) {
    ...
    private void saveWorldValues(File directory) {
        Registry<PlacedFeature> registry = getRegistry().lookupOrThrow(Registries.PLACED_FEATURE);
        ...
        forEachBiome((biome, biomeKey) -> {
            ...
            for (HolderSet<PlacedFeature> list : decorations) {
                for (Holder<PlacedFeature> featureSupplier : list) {
                    ...
                    Registry<PlacedFeature> registry = getRegistry().lookupOrThrow(Registries.PLACED_FEATURE);
                    Identifier featureKey = registry.getKey(feature);
                    ...
                    File featureFile = new File(
                            biomeFile,
                            featureKey.getNamespace() + "/" + featureKey.getPath() + ".json");
                    try {
                        saveFeature(
                                featureFile,
                                registries.getBiomeRegistry().get(CraftNamespacedKey.fromMinecraft(biomeKey)).get(),
                                CraftNamespacedKey.fromMinecraft(featureKey),
                                feature);
                    } catch (Exception e) {
                        throw new RuntimeException(
                                String.format(
                                        "Error while saving feature '%s' in biome '%s'",
                                        featureKey,
                                        biomeKey), e);
                    }
                }
    ...
    private void saveFeature(
            File file,
            de.derfrzocker.ore.control.api.Biome biome,
            NamespacedKey featureKey,
            PlacedFeature feature) {
        ConfiguredFeature<?, ?> configuredFeature = feature.getFeatures().findFirst().map(Holder::value).orElse(null);
        ...
        if (!(configuredFeature.feature() instanceof OreFeature) &&
            !(configuredFeature.feature() instanceof ScatteredOreFeature) &&
            !(configuredFeature.feature() instanceof GlowstoneFeature)) {
            return;
        }
        ...
        if (configuredFeature.feature() instanceof OreFeature) {
            ...
            featureConfiguration = OreConfigurationGeneratorHook.createDefaultConfiguration(
                    configuration,
                    (FeatureGenerator<OreFeatureConfiguration>) featureGenerator);
        } else if (configuredFeature.feature() instanceof ScatteredOreFeature) {
            OreConfiguration configuration = (OreConfiguration) configuredFeature.config();
            featureGenerator = registries
                    .getFeatureGeneratorRegistry()
                    .get(NamespacedKey.minecraft("scattered_ore"))
                    .get();
            featureConfiguration = OreConfigurationGeneratorHook.createDefaultConfiguration(
                    configuration,
                    (FeatureGenerator<OreFeatureConfiguration>) featureGenerator);
            ...
            featureGenerator = registries
                    .getFeatureGeneratorRegistry()
                    .get(NamespacedKey.minecraft("glowstone_blob"))
                    .get();
        ...
        for (PlacementModifier placement : feature.placement()) {
            Identifier placementModifierType = placementModifierTypes.getKey(placement.type());
            Optional<FeaturePlacementModifier<?>> modifierOptional = registries
                    .getPlacementModifierRegistry()
                    .get(NamespacedKey.fromString(placementModifierType.toString()));
            ...
            if (placement.type() == PlacementModifierType.RARITY_FILTER && placement instanceof RarityFilter) {
                placementConfiguration.add(RarityModifierHook.createDefaultConfiguration(
                        (RarityFilter) placement,
                        modifierOptional.get()));
            }
            if (placement.type() == PlacementModifierType.SURFACE_RELATIVE_THRESHOLD_FILTER &&
                placement instanceof SurfaceRelativeThresholdFilter) {
                placementConfiguration.add(SurfaceRelativeThresholdModifierHook.createDefaultConfiguration(
                        (SurfaceRelativeThresholdFilter) placement,
                        modifierOptional.get()));
            }
            if (placement.type() == PlacementModifierType.SURFACE_WATER_DEPTH_FILTER &&
                placement instanceof SurfaceWaterDepthFilter) {
                placementConfiguration.add(SurfaceWaterDepthModifierHook.createDefaultConfiguration(
                        (SurfaceWaterDepthFilter) placement,
                        modifierOptional.get()));
            }
            if (placement.type() == PlacementModifierType.COUNT && placement instanceof CountPlacement) {
                placementConfiguration.add(CountModifierHook.createDefaultConfiguration(
                        (CountPlacement) placement,
                        modifierOptional.get()));
                hasCount = true;
            }
            if (placement.type() == PlacementModifierType.HEIGHT_RANGE && placement instanceof HeightRangePlacement) {
                placementConfiguration.add(HeightRangeModifierHook.createDefaultConfiguration(
        ...
        if (!hasCount) {
            ...
            modifierOptional.ifPresent(modifier -> placementConfiguration.add(
                    0,
                    CountModifierHook.createDefaultConfiguration(
                            CountPlacement.of(1),
                            modifier)));
        }
        ...
        modifierOptional.ifPresent(modifier -> placementConfiguration.add(
                0,
                new ActivationConfiguration(
                        modifier,
                        new FixedBooleanValue(true))));
    ...
    private void forEachBiome(BiConsumer<Biome, Identifier> consumer) {
        Registry<Biome> registry = getRegistry().lookupOrThrow(Registries.BIOME);
    ...
    private void hookIntoBiome(@NotNull final Biome biome, Identifier minecraftKey)
            throws NoSuchFieldException, IllegalAccessException {
        de.derfrzocker.ore.control.api.Biome oreControlBiome = registries
                .getBiomeRegistry()
                .get(CraftNamespacedKey.fromMinecraft(minecraftKey))
                .get();
        List<HolderSet<PlacedFeature>> decorations = biome.getGenerationSettings().features();
...

        {
            final Field field = getField(
                    ...
                    NMSReflectionNames.BIOME_GENERATION_SETTINGS_FEATURES);
        ...
        }
        {
            final Field field = getField(
                    BiomeGenerationSettings.class,
                    NMSReflectionNames.BIOME_GENERATION_SETTINGS_FEATURE_SET);
            field.setAccessible(true);
            field.set(
                    biome.getGenerationSettings(),
                    Suppliers.memoize(() -> newDecorations
                            .stream()
    ...
    @Nullable
    private PlacedFeature check(
            de.derfrzocker.ore.control.api.Biome biome,
            Identifier biomeKey,
            PlacedFeature feature) {
        // Not the best method to do this, but hey it works
        final Registry<PlacedFeature> registry = getRegistry().lookupOrThrow(Registries.PLACED_FEATURE);
        Identifier featureKey = registry.getKey(feature);

        // Return if the featureKey is null, this can happen when plugins are adding placed features to the biome
        ...
        for (PlacementModifier placement : feature.placement()) {
            if (placement.type() == PlacementModifierType.RARITY_FILTER) {
                ...
                placementModifiers.add(new SurfaceRelativeThresholdModifierHook(
                        oreControlManager,
                        biome,
                        key,
                        (SurfaceRelativeThresholdFilter) placement));
            } else if (placement.type() == PlacementModifierType.SURFACE_WATER_DEPTH_FILTER) {
                placementModifiers.add(new SurfaceWaterDepthModifierHook(
                        oreControlManager,
                        biome,
                        key,
                        (SurfaceWaterDepthFilter) placement));
                ...
                hasCount = true;
                placementModifiers.add(new CountModifierHook(
                        oreControlManager,
                        biome,
                        key,
                        (CountPlacement) placement));
            } else if (placement.type() == PlacementModifierType.HEIGHT_RANGE) {
                placementModifiers.add(new HeightRangeModifierHook(
                        oreControlManager,
                        biome,
                        key,
                        (HeightRangePlacement) placement));
        ...
        Optional<FeaturePlacementModifier<?>> modifierOptional = registries
                .getPlacementModifierRegistry()
                .get(ActivationModifier.KEY);
        modifierOptional.ifPresent(modifier -> placementModifiers.add(
                0,
                new ActivationModifierHook(
                        oreControlManager,
                        biome,
                        key)));
        ...
        if (configuredFeature.feature() instanceof OreFeature) {
            return new PlacedFeature(
                    Holder.direct(new ConfiguredFeature(
                            OreConfigurationGeneratorHook.createOreHook(
                                    oreControlManager,
                                    key,
                                    biome), configuredFeature.config())), placementModifiers);
        } else if (configuredFeature.feature() instanceof ScatteredOreFeature) {
            return new PlacedFeature(
                    Holder.direct(new ConfiguredFeature(
                            OreConfigurationGeneratorHook.createScatteredOreHook(
                                    oreControlManager,
                                    key,
                                    biome), configuredFeature.config())), placementModifiers);
        } else if (configuredFeature.feature() instanceof GlowstoneFeature) {
            return new PlacedFeature(
                    Holder.direct(new ConfiguredFeature(
                            EmptyConfigurationGeneratorHook.createGlowstoneBlobHook(
                                    oreControlManager,
                                    key,
                                    biome), configuredFeature.config())), placementModifiers);
        }
